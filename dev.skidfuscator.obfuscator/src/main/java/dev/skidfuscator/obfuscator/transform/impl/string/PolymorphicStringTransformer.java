...
public class PolymorphicStringTransformer extends AbstractTransformer {
    private final Map<SkidClassNode, EncryptionGeneratorV3> keyMap = new HashMap<>();

    private final Set<String> INJECTED = new HashSet<>();

    public PolymorphicStringTransformer(Skidfuscator skidfuscator) {
        this(skidfuscator, Collections.emptyList());
    }

    public PolymorphicStringTransformer(Skidfuscator skidfuscator, List<Transformer> children) {
        super(skidfuscator, "Polymorphic String Encryption", children);
    }
    ...
    void handle(final RunMethodTransformEvent event) {
        ...
        cfg.allExprStream()
                ...
                .forEach(unit -> {
                    ...
                    final Expr modified = generator.encrypt(constant, methodNode, (SkidBlock) unit.getBlock());
